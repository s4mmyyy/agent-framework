...
class MySimpleAgent(SimpleAgent):
    ...
    def _get_ehanced_system_prompt(self) -> str:
        """构建增强的系统提示词，包含工具信息"""
        ...
        tools_section += "例如：`[TOOL_CALL:search:Python编码]` 或 `[TOOL_CALL:memory:recall=用户信息]` \n\n"
        tools_section += "工具调用结果会自动插入到对话中，然后你可以基于结果继续回答问题。\n"
    ...
    def _parse_tool_parameters(self,tool_name: str,parameters: str) -> dict:
        ...
        if '=' in parameters:
            #格式： key=value 或 actiobn=search, query=Python
            if ',' in parameters:
    ...
    def stream_run(self, input_text, **kwargs) -> Iterator[str] :
        ...
        for msg in self._history:
            messages.append({"role":msg.role,"conetnt":msg.content})
